...
export async function checkConf(page: Page, label: string) {
    ...
    await expect(confLocator.locator('button:has-text("Start")')).toBeVisible();
    await expect(confLocator.locator('svg[data-testid="MobileScreenShareIcon"]')).toBeVisible();
    await expect(confLocator.locator('svg[data-testid="ExpandLessIcon"]')).toBeVisible();
